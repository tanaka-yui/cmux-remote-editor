...
- `components/ConnectionIndicator.tsx` / `components/Header.tsx` — **旧 footer(StatusBar) は廃止**。接続状態（ドット＋ラベル、`connected→切断` のみ 2 秒猶予でチラつき防止）とオフライン/履歴の鮮度表示（「最終 HH:MM」/「履歴 · HH:MM時点」）を `ConnectionIndicator` に抽出し、Header の「履歴」ボタンの隣へ移設。ペイン名/位置ドットは廃止。Header には**歯車（設定）ボタン**もあり `SettingsModal` を開く。
- `components/SettingsModal.tsx` / `lib/settings.ts` — 設定モーダル。今は**履歴バッファ（スクロールバック行数）**のみ調整可（スライダー＋数値入力、`HISTORY_LINES_MIN`〜`MAX`=1000〜100000、既定 2000）。`localStorage`(`cmux:history-lines`)に永続し、App の履歴モード `readText(..., { scrollback: true, lines })` に渡る。
